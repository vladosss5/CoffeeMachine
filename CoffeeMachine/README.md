...
To update the database use this command from the Web project folder (replace `CoffeeMachine` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../CoffeeMachine.Infrastructure/CoffeeMachine.Infrastructure.csproj -s CoffeeMachine.Web.csproj
```
